<template>
  <div>
    <div class="comment">
      ...
      <el-button type="primary" class="sub-btn" @click="postComment()">Submit</el-button>
    </div>
    <p>Comment: total of {{commentList.length}} comments</p>
        ...
        setComment(params)
          .then(res => {
            if (res.code === 1) {
              this.textarea = ''
              this.getComment()
              this.notify('comment successfully', 'success')
            } else {
              this.notify('comment failed', 'error')
            }
          })
          .catch(err => {
            console.log(err)
